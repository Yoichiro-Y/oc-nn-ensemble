...
class OneClassNeuralNetwork:
    ...
    def custom_ocnn_loss(self, nu, xi, w, V):
        def custom_hinge(_, y_pred):
            loss = 0.5 * tf.reduce_sum(w ** 2) + 0.5 * tf.reduce_sum(V ** 2) + quantile_loss(self.r, y_pred, xi)
    ...
    def train_model(self, X, epochs=50, nu=0.1, xi=0.1, init_lr=1e-2, save=True):
        """
        ...
        model.compile(optimizer=Adam(lr=init_lr, decay=init_lr / epochs),
                      loss=self.custom_ocnn_loss(nu, xi, w, V), metrics=[r_metric, quantile_loss_metric], run_eagerly=True)
    ...
    def load_model(self, model_dir):
        ...
        model = load_model(f'{model_dir}/model.h5',
                           custom_objects={'custom_hinge': self.custom_ocnn_loss(nu, xi, w, V)})
        return model
