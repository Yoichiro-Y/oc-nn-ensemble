...
def main():
    ...
    _, test_acc = oc_nn.evaluate(x_mixed_test, y_mixed_test)
    print('Test accuracy:', test_acc)

    auc = tf.metrics.AUC()
    auc.update_state(y_mixed_test, y_pred)
    print('AUC:', auc.result().numpy())

    y_pred = np.where(y_pred > 0.5, 1, 0)
    cm = tf.math.confusion_matrix(labels=y_mixed_test, predictions=y_pred)
