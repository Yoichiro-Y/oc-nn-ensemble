...
def main():
    ...
    x_normal_test = x_test[np.where(y_test==normal_idx,True,False)][:800]
    x_abnormal_train = x_train[np.where(y_train==abnormal_idx,True,False)]
    x_abnormal_test = x_test[np.where(y_test==abnormal_idx,True,False)][:200]

    auc_list_list = []
    sum_list = []

    train_anomaly_rate = 0.2
    ...
    for _ in range(10):
        ...
        nu_list = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5]
